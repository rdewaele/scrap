...
if !exists("g:tex_indent_sections")
		let g:tex_indent_sections = 1
endif
...
if !exists("g:tex_section")
		let g:tex_section = '\\section{.*}'
		let g:tex_section_i = 0 * &sw
endif
if !exists("g:tex_subsection")
		let g:tex_subsection = '\\subsection{.*}'
		let g:tex_subsection_i = 1 * &sw
endif
if !exists("g:tex_subsubsection")
		let g:tex_subsubsection = '\\subsubsection{.*}'
		let g:tex_subsubsection_i = 2 * &sw
endif
if !exists("g:tex_no_indents")
		let g:tex_no_indents = '^\s*\\appendix\|^\s*\\clearpage'
endif
...
function GetTeXIndent()
    ...
    " ignore unindented lines: comments (never adjusted), and g:tex_no_indents
		" (always left justified)
    while lnum != 0 && (getline(lnum) =~ '^\s*%' || getline(lnum) =~ g:tex_no_indents)
    ...
    if cline =~ '^\s*%'
		...
		" no indentation special cases
		if cline =~ g:tex_no_indents
				return 0
		endif
    ...
    if g:tex_indent_brace
		...
		" Add or subtract a 'shiftwidth' after a section
		if g:tex_indent_sections
				if cline =~ g:tex_section
						let ind = g:tex_section_i
				elseif cline =~ g:tex_subsection
						let ind = g:tex_subsection_i
				elseif cline =~ g:tex_subsubsection
						let ind = g:tex_subsubsection_i
				endif
		endif
